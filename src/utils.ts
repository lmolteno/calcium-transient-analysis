...
export const formatSeconds = (seconds: number, decimalPrecision = 0) => {
  ...
  if (hours > 1) {
    return [
      String(hours),
      String(leftOverMinutes).padStart(2, "0"),
      String(leftOverSeconds.toFixed(decimalPrecision)).padStart(2, "0"),
    ]
      .filter((d) => d)
      .join(":");
  }
...
export const exportCells = (
  filename: string,
  cellsAndSections: CellAndSections[],
) => {
      ...
      .map((c) =>
        [
          ...
          ...c.sections.flatMap((s) => [
            s.area.toString(),
            s.peaks.toString(),
            s.totalPeakTime.toString(),
            s.proportionPeakTime.toString(),
          ]),
        ].join(","),
      )
      .join("\n");
...
export const calculatePeaks = (data: Datum[], threshold = 1): Peak[] => {
  ...
  for (let i = 1; i < len; i++) {
    ...
    if (prevY < threshold && currY > threshold) {
      const intersection = getIntersectionX(
        prevX,
        prevY,
        currX,
        currY,
...
export const filterPeaksToSection = (peaks: Peak[], section: Section) =>
  peaks.flatMap((p) => {
    ...
    if (!endIncluded && startIncluded)
      return [{ ...p, end: section.end, length: section.end - p.start }];
    if (endIncluded && !startIncluded)
      return [{ ...p, start: section.start, length: p.end - section.start }];
    return [];
  });
